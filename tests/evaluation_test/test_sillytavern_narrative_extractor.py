...
from world_simulation_engine.component.sillytavern_converter.narrative_extractor import NarrativeExtractor
...
async def test_extract_narrative_from_real_sillytavern_card(card_path: Path, global_scope_chat_config):
    ...
    extraction = await NarrativeExtractor(database=global_scope_chat_config).extract(
    ...
    for event in extraction.events:
        assert event.involved_character_ids
        assert set(event.involved_character_ids) <= known_ids
    for memory in extraction.memories:
        assert set(memory.character_ids) <= known_ids
        assert memory.event_id in {event.id for event in extraction.events}
    for relationship in extraction.relationships:
        assert relationship.source_character_id in known_ids
        assert relationship.target_character_id in known_ids
        assert relationship.source_character_id != relationship.target_character_id

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    (OUTPUT_DIR / f"narrative_extractor_{card_path.stem}_results.json").write_text(
        json.dumps(
            {
                "card": card_path.stem,
                "events": [event.model_dump() for event in extraction.events],
                "memories": [memory.model_dump() for memory in extraction.memories],
