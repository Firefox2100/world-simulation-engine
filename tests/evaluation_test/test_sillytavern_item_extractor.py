...
from world_simulation_engine.component.sillytavern_converter.item_extractor import ItemExtractor
...
async def global_scope_chat_config(evaluation_database, evaluation_connection_config, evaluation_chat_model_config):
    ...
    await evaluation_database.config.link_global_chat(
        evaluation_chat_model_config.id, ComponentType.ST_ITEM_EXTRACTOR,
    )
...
async def test_extract_items_from_real_sillytavern_card(card_path: Path, global_scope_chat_config):
    ...
    the `item` bucket (SILLYTAVERN_IMPORT_PLAN.md §5) - a card with no dedicated item-describing
    lorebook content is expected to yield zero items, not an error."""
    ...
    extraction = await ItemExtractor(database=global_scope_chat_config).extract(
    ...
    (OUTPUT_DIR / f"item_extractor_{card_path.stem}_results.json").write_text(
        json.dumps(
            {
                "card": card_path.stem,
