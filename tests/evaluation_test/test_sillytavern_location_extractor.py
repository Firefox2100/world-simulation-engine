...
from world_simulation_engine.component.sillytavern_converter.location_extractor import LocationExtractor
...
async def global_scope_chat_config(evaluation_database, evaluation_connection_config, evaluation_chat_model_config):
    ...
    await evaluation_database.config.link_global_chat(
        evaluation_chat_model_config.id, ComponentType.ST_LOCATION_EXTRACTOR,
    )
...
async def test_extract_locations_from_real_sillytavern_card(card_path: Path, global_scope_chat_config):
    ...
    extraction = await LocationExtractor(database=global_scope_chat_config).extract(
        preprocessed, classification, language=language,
    )

    # Unlike characters, a card legitimately can yield zero locations (e.g. no location-bucket
    # items and no concrete place implied by scenario/first_message) - only shape/consistency is
    # asserted, not a minimum count.
    ids = [location.id for location in extraction.locations]
    assert len(ids) == len(set(ids))
    for location in extraction.locations:
        assert location.name.strip()
        assert location.description.strip()
        if location.parent_id is not None:
            assert location.parent_id in ids

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    (OUTPUT_DIR / f"location_extractor_{card_path.stem}_results.json").write_text(
        json.dumps(
            {
                "card": card_path.stem,
                "locations": [location.model_dump() for location in extraction.locations],
            },
            indent=2,
            ensure_ascii=False,
        ),
