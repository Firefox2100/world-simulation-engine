...
async def test_build_perspective_resolves_placeholder_references_without_mutating_source():
    world = World(
        id="world_1",
        name="World",
        description="A world",
    ...
    character = Character(
        ...
        gender="unknown",
        appearance="Plain",
        description="Old friends with {{ character['character_2'].name }}.",
        public_state="Standing",
        private_state="Focused",
        current_activity={"name": "idle"},
    )
    ...
    simulator = CharacterSimulator(database=database, langfuse_handler=None)
    simulator._perspective_resolver.resolve_perceived_entities = AsyncMock(return_value=Mock(
        perceived_characters=[PerceivedCharacter(character=other, visibility=Visibility.VISIBLE)],
        perceived_background_characters=[],
        perceived_items=[],
        perceived_equipment=[],
        perceived_containers=[],
    ...
    assert perspective.actor.description == "Old friends with Blair."
    assert character.description == "Old friends with {{ character['character_2'].name }}."
