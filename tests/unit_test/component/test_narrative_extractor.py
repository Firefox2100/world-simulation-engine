...
def make_character(name: str, char_id: str, do_not_know=None) -> ExtractedCharacter:
...
async def test_extract_resolves_names_to_ids_and_respects_do_not_know():
    card = make_card()
...
async def test_extract_falls_back_to_involved_ids_when_knowing_names_empty():
    card = make_card()
    classification = LorebookClassification(items=[
        ClassifiedItem(item_id="entry:1", buckets=[LorebookItemBucket.HISTORY_EVENT]),
    ])
    ...
    assert extraction.memories[0].character_ids == ["id-alice"]
