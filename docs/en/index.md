...
This software uses complicated multi-agent architecture and structured states based on graph database to try to maintain the consistency and continuity of a simulated session. Some important features include:
...
- **Structured states**: The software maintains a structured state of the simulation, stored into the graph database, instead of just the narrated text. This allows for more complex and realistic management of the state. The state simulation and frontend rendering is decoupled, so that the state can be rendered in different ways.
- **Time and perspective based simulation**: The simulation is driven by time, and each character gets a perspective of the previous events and world state. This allows more realistic and immersive simulations, as characters can have different knowledge and understanding of the world based on their perspective, and has internal trigger to act even when the user is not interacting with them.
- **First class support for media generation**: The software has first class support for media generation, including image, video (WIP) and audio. This allows for more immersive and engaging simulations.
